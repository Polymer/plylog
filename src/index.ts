...
import {Logger, LoggerInstance, ConsoleTransportInstance, transports} from 'winston';

class PolymerLogger {

  _transport: ConsoleTransportInstance;
  ...
  constructor(options?: {level?: string, name?: string}) {
    ...
    this._transport = new (transports.Console)({
      level: options.level || 'info',
      label: options.name || null,
      prettyPrint: true,
    });
    this._logger = new (Logger)({transports: [this._transport]});
    this._logger.cli();
  }
   ...
   */
  _log(_level:string, _msg: string, ..._metadata: any[]): void {
   ...
   */
  error(_msg: string, ..._metadata: any[]): void {
   ...
   */
  warn(_msg: string, ..._metadata: any[]): void {
   ...
   */
  info(_msg: string, ..._metadata: any[]): void {
   ...
   * issue.
   */
  debug(_msg: string, ..._metadata: any[]): void {
